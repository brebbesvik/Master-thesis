...
We can further expand the model by looking at the entity model and the workflow model combined. Then we can identify the very specific resources for each content unit. Se figure \ref{fig:ExpandedKnowledgeMap}. By putting investigation and surgery into the model, we have identified some vertices which are not relevant for the the paediatric possible asthma guideline \parencite{RepublicofKeny2016}, but is relevant when making courses for other guidelines. The repository and the knowledge map will expand the more we learn about other guidelines.

\begin{figure}[h!]
	\caption {The knowledge map has been expanded to identify the specific resources for each content unit. The content units are the parent nodes, while the leaves are the resources}
